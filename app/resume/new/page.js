...
export default function NewResumePage() {
  ...
  const generateWithAI = async () => {
    if (!aiInput.jobTitle.trim()) { alert('Enter job title'); return }
    setAiLoading(true)
    try {
      ...
      if (result.success && result.data) {
        ...
        alert('Resume generated! Edit any field to customize.')
      } else alert('AI failed: ' + (result.error || 'Unknown'))
  ...
  const checkATSScore = async () => {
    ...
    } catch (error) { alert('Error: ' + error.message); setShowATSModal(false) }
  ...
  const autoImproveResume = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setResume({
          personal: improved.personal || resume.personal,
          summary: improved.summary || resume.summary,
          experience: (improved.experience || []).map(e => ({ ...e, id: Date.now() + Math.random() })),
          education: (improved.education || []).map(e => ({ ...e, id: Date.now() + Math.random() })),
          skills: improved.skills || resume.skills,
          projects: (improved.projects || []).map(p => ({ ...p, id: Date.now() + Math.random() }))
        })
        setShowATSModal(false)
        alert('✨ Resume improved! Click ATS Score again to see new rating.')
      } else alert('Improvement failed')
    } catch (error) { alert('Error: ' + error.message) }
  ...
  const parseUploadedResume = async () => {
    if (!uploadText.trim() || uploadText.length < 50) {
      alert('Please paste your full resume content (at least 50 characters)')
      return
    }
    ...
    try {
      const response = await fetch('/api/ai/parse', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ resumeText: uploadText })
      })
      const result = await response.json()
      if (result.success && result.data) {
        const parsed = result.data
        setResume({
          personal: { ...parsed.personal, email: parsed.personal.email || user.email },
          summary: parsed.summary || '',
          experience: (parsed.experience || []).map(e => ({ ...e, id: Date.now() + Math.random() })),
          education: (parsed.education || []).map(e => ({ ...e, id: Date.now() + Math.random() })),
          skills: parsed.skills || [],
          projects: (parsed.projects || []).map(p => ({ ...p, id: Date.now() + Math.random() }))
        })
        setShowUploadModal(false)
        setUploadText('')
        alert('✨ Resume imported and improved! Review and edit any field.')
      } else alert('Parsing failed: ' + (result.error || 'Unknown'))
    } catch (error) { alert('Error: ' + error.message) }
  ...
  const saveResume = async () => {
    ...
    const { error } = await supabase.from('resumes').insert({
      user_id: user.id, title: resumeTitle, template_id: 'modern', content: resume
    })
    setSaving(false)
    if (error) alert('Error: ' + error.message)
    else { alert('Resume saved!'); router.push('/dashboard') }
  }
    ...
    <div style={{ fontFamily: 'system-ui', background: '#f5f7fa', minHeight: '100vh' }}>
      <header style={{ background: 'white', padding: '15px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', position: 'sticky', top: 0, zIndex: 100, flexWrap: 'wrap', gap: '10px' }} className="no-print">
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={() => setShowUploadModal(true)} style={{ padding: '10px 16px', background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>📤 Upload CV</button>
          <button onClick={() => setShowAIModal(true)} style={{ padding: '10px 16px', background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a6f 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>🤖 AI Generate</button>
          <button onClick={checkATSScore} style={{ padding: '10px 16px', background: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>🎯 ATS Score</button>
          <button onClick={downloadPDF} style={{ padding: '10px 16px', background: '#f5f5f5', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>📥 PDF</button>
          <button onClick={saveResume} disabled={saving} style={{ padding: '10px 20px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>{saving ? 'Saving...' : '💾 Save'}</button>
        </div>
      </header>
      ...
      {showUploadModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '15px', padding: '30px', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ color: '#1a1a2e', marginBottom: '10px' }}>📤 Upload Existing Resume</h2>
            <p style={{ color: '#666', marginBottom: '20px', fontSize: '14px' }}>
              Open your existing resume (PDF/Word), select all text (Ctrl+A), copy it (Ctrl+C), and paste below. AI will extract everything and improve it automatically!
            </p>
            ...
            <div style={{ background: '#f0f9ff', padding: '15px', borderRadius: '10px', marginBottom: '15px', border: '1px solid #bae6fd' }}>
              <p style={{ color: '#0369a1', fontSize: '13px', margin: 0 }}>
                ...
                1. Open your old resume file (PDF/Word/Google Docs)<br/>
                2. Press Ctrl+A to select all text<br/>
                3. Press Ctrl+C to copy<br/>
                4. Click in box below and press Ctrl+V to paste
              </p>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              <button onClick={() => { setShowUploadModal(false); setUploadText('') }} style={{ flex: 1, padding: '12px', background: '#f5f5f5', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>Cancel</button>
              <button onClick={parseUploadedResume} disabled={parseLoading} style={{ flex: 2, padding: '12px', background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
                {parseLoading ? '✨ Importing & Improving...' : '✨ Import & Improve with AI'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showAIModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '15px', padding: '30px', maxWidth: '500px', width: '100%' }}>
            ...
            <p style={{ color: '#666', marginBottom: '20px' }}>Tell AI about you and get instant resume</p>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '15px', padding: '30px', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {atsLoading ? (
              ...
              <div>
                <div style={{ textAlign: 'center', marginBottom: '20px', padding: '30px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', borderRadius: '15px', color: 'white' }}>
                ...
                {/* AUTO IMPROVE BUTTON - PROMINENT */}
                <div style={{ background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', padding: '20px', borderRadius: '15px', marginBottom: '20px', textAlign: 'center', color: 'white' }}>
                  <h3 style={{ marginBottom: '10px', fontSize: '1.2rem' }}>✨ One-Click Magic</h3>
                  <p style={{ marginBottom: '15px', opacity: 0.95, fontSize: '14px' }}>
                    Let AI automatically fix all issues and improve your resume
                  </p>
                  <button 
                    ...
                    {improveLoading ? '✨ Improving...' : '🚀 Auto-Improve My Resume'}
                  </button>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '20px', maxWidth: '1600px', margin: '0 auto' }} className="editor-grid">
          ...
          <div style={{ padding: '25px' }}>
            {activeSection === 'personal' && (
              <div>
                ...
                <label style={labelStyle}>Location</label><input type="text" value={resume.personal.location} onChange={(e) => updatePersonal('location', e.target.value)} placeholder="Mumbai" style={inputStyle} />
              ...
              <div>
                ...
                {resume.experience.map((exp, index) => (
                  <div key={exp.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <label style={labelStyle}>Role</label><input type="text" value={exp.role} onChange={(e) => updateExperience(exp.id, 'role', e.target.value)} placeholder="Engineer" style={inputStyle} />
            ...
            {activeSection === 'education' && (
              <div>
                ...
                {resume.education.length === 0 && <p style={{ color: '#999', textAlign: 'center', padding: '40px' }}>No education added.</p>}
                {resume.education.map((edu, index) => (
                  <div key={edu.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <label style={labelStyle}>Degree</label><input type="text" value={edu.degree} onChange={(e) => updateEducation(edu.id, 'degree', e.target.value)} placeholder="B.E" style={inputStyle} />
            ...
            {activeSection === 'skills' && (
              <div>
                ...
                <p style={{ color: '#666', fontSize: '14px', marginBottom: '20px' }}>Type and press Enter</p>
                <input type="text" placeholder="React..." onKeyDown={(e) => { if (e.key === 'Enter' && e.target.value.trim()) { addSkill(e.target.value.trim()); e.target.value = '' } }} style={inputStyle} />
            ...
            {activeSection === 'projects' && (
              <div>
                ...
                {resume.projects.length === 0 && <p style={{ color: '#999', textAlign: 'center', padding: '40px' }}>No projects.</p>}
                {resume.projects.map((proj, index) => (
                  <div key={proj.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <label style={labelStyle}>Tech</label><input type="text" value={proj.tech} onChange={(e) => updateProject(proj.id, 'tech', e.target.value)} placeholder="React" style={inputStyle} />
                    ...
                    <label style={labelStyle}>Description</label>
                    <textarea value={proj.description} onChange={(e) => updateProject(proj.id, 'description', e.target.value)} placeholder="Description" style={{ ...inputStyle, minHeight: '80px' }} />
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ background: 'white', borderRadius: '15px', padding: '40px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', minHeight: '800px' }} id="resume-preview">
          <div style={{ borderBottom: '3px solid #667eea', paddingBottom: '20px', marginBottom: '25px' }}>
            ...
            <p style={{ fontSize: '1.2rem', color: '#667eea', marginBottom: '15px', fontWeight: '500' }}>{resume.personal.jobTitle || 'Job Title'}</p>
              ...
              {resume.experience.map(exp => (
                <div key={exp.id} style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div><h3 style={{ color: '#1a1a2e', fontSize: '1.1rem' }}>{exp.role}</h3><p style={{ color: '#667eea', fontWeight: '500' }}>{exp.company}</p></div>
