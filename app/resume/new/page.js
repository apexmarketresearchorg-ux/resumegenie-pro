...
export default function NewResumePage() {
  ...
  const checkUser = async () => {
    ...
    setUser(user)
    setResume(prev => ({ ...prev, personal: { ...prev.personal, email: user.email } }))
    setLoading(false)
  }
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        alert('Resume generated successfully! Edit any field to customize.')
      } else {
  ...
  const addEducation = () => {
    setResume(prev => ({
      ...prev,
      education: [...prev.education, { id: Date.now(), institution: '', degree: '', field: '', startDate: '', endDate: '', grade: '' }]
    }))
  }
  ...
  const addProject = () => {
    setResume(prev => ({
      ...prev,
      projects: [...prev.projects, { id: Date.now(), name: '', description: '', link: '', tech: '' }]
    }))
  }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', fontFamily: 'system-ui' }}>
        Loading...
      </div>
    )
  }
    ...
    <div style={{ fontFamily: 'system-ui', background: '#f5f7fa', minHeight: '100vh' }}>
      <header style={{ background: 'white', padding: '15px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', position: 'sticky', top: 0, zIndex: 100, flexWrap: 'wrap', gap: '10px' }} className="no-print">
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <button onClick={() => setShowAIModal(true)} style={{ padding: '10px 20px', background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a6f 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
            🤖 AI Generate
          </button>
          <button onClick={checkATSScore} style={{ padding: '10px 20px', background: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
            🎯 ATS Score
          </button>
          <button onClick={downloadPDF} style={{ padding: '10px 20px', background: '#f5f5f5', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
            📥 PDF
          </button>
          <button onClick={saveResume} disabled={saving} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
            {saving ? 'Saving...' : '💾 Save'}
          </button>
        </div>
      </header>
      ...
      {showAIModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '15px', padding: '30px', maxWidth: '500px', width: '100%' }}>
            ...
            <p style={{ color: '#666', marginBottom: '20px' }}>Tell AI about you and get instant professional resume</p>
            
            <label style={labelStyle}>Job Title You Want *</label>
            <input type="text" value={aiInput.jobTitle} onChange={(e) => setAiInput({ ...aiInput, jobTitle: e.target.value })} placeholder="e.g., Software Engineer, Marketing Manager" style={inputStyle} />

            <label style={labelStyle}>Years of Experience</label>
            <input type="text" value={aiInput.experience} onChange={(e) => setAiInput({ ...aiInput, experience: e.target.value })} placeholder="e.g., 3 years or Fresher" style={inputStyle} />

            <label style={labelStyle}>Your Key Skills (comma separated)</label>
            <input type="text" value={aiInput.skills} onChange={(e) => setAiInput({ ...aiInput, skills: e.target.value })} placeholder="e.g., React, Python, Communication" style={inputStyle} />
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              ...
              <button onClick={generateWithAI} disabled={aiLoading} style={{ flex: 2, padding: '12px', background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a6f 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
            ...
            {atsLoading ? (
              ...
              <div>
                ...
                {atsResult.strengths && (
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <ul style={{ paddingLeft: '20px' }}>
                      {atsResult.strengths.map((s, i) => <li key={i} style={{ color: '#333', marginBottom: '5px' }}>{s}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {atsResult.improvements && (
                  <div style={{ marginBottom: '20px' }}>
                    <h3 style={{ color: '#ee5a6f', marginBottom: '10px' }}>🔧 Improvements Needed</h3>
                    <ul style={{ paddingLeft: '20px' }}>
                      {atsResult.improvements.map((s, i) => <li key={i} style={{ color: '#333', marginBottom: '5px' }}>{s}</li>)}
                    </ul>
                  </div>
                )}
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                      {atsResult.missingKeywords.map((k, i) => (
                        <span key={i} style={{ background: '#fff3e0', color: '#e65100', padding: '6px 12px', borderRadius: '15px', fontSize: '13px' }}>{k}</span>
                      ))}
                    </div>
                  </div>
                )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '20px', maxWidth: '1600px', margin: '0 auto' }} className="editor-grid">
        ...
        <div style={{ background: 'white', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }} className="no-print">
          <div style={{ display: 'flex', background: '#f8f9fa', borderBottom: '1px solid #e0e0e0', overflowX: 'auto' }}>
            {sections.map(s => (
              <button key={s.id} onClick={() => setActiveSection(s.id)} style={{ padding: '15px 20px', background: activeSection === s.id ? 'white' : 'transparent', border: 'none', borderBottom: activeSection === s.id ? '3px solid #667eea' : '3px solid transparent', cursor: 'pointer', fontSize: '14px', fontWeight: '500', whiteSpace: 'nowrap', color: activeSection === s.id ? '#667eea' : '#666' }}>
                {s.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ padding: '25px' }}>
            {activeSection === 'personal' && (
              <div>
                ...
                <input type="text" value={resume.personal.fullName} onChange={(e) => updatePersonal('fullName', e.target.value)} placeholder="John Doe" style={inputStyle} />
                <label style={labelStyle}>Job Title *</label>
                <input type="text" value={resume.personal.jobTitle} onChange={(e) => updatePersonal('jobTitle', e.target.value)} placeholder="Software Engineer" style={inputStyle} />
                <label style={labelStyle}>Email *</label>
                <input type="email" value={resume.personal.email} onChange={(e) => updatePersonal('email', e.target.value)} placeholder="john@example.com" style={inputStyle} />
                <label style={labelStyle}>Phone</label>
                <input type="text" value={resume.personal.phone} onChange={(e) => updatePersonal('phone', e.target.value)} placeholder="+91 9876543210" style={inputStyle} />
                <label style={labelStyle}>Location</label>
                <input type="text" value={resume.personal.location} onChange={(e) => updatePersonal('location', e.target.value)} placeholder="Mumbai, India" style={inputStyle} />
                <label style={labelStyle}>LinkedIn</label>
                <input type="text" value={resume.personal.linkedin} onChange={(e) => updatePersonal('linkedin', e.target.value)} placeholder="linkedin.com/in/johndoe" style={inputStyle} />
                <label style={labelStyle}>Website</label>
                <input type="text" value={resume.personal.website} onChange={(e) => updatePersonal('website', e.target.value)} placeholder="johndoe.com" style={inputStyle} />
              </div>
            )}
            ...
            {activeSection === 'summary' && (
              <div>
                ...
                <p style={{ color: '#666', fontSize: '14px', marginBottom: '20px' }}>Or click "AI Generate" button above to auto-fill!</p>
                <textarea value={resume.summary} onChange={(e) => setResume(prev => ({ ...prev, summary: e.target.value }))} placeholder="Experienced professional with..." style={{ ...inputStyle, minHeight: '200px' }} />
              </div>
            )}
              ...
              <div>
                ...
                {resume.experience.map((exp, index) => (
                  <div key={exp.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <input type="text" value={exp.company} onChange={(e) => updateExperience(exp.id, 'company', e.target.value)} placeholder="Google" style={inputStyle} />
                    <label style={labelStyle}>Role</label>
                    <input type="text" value={exp.role} onChange={(e) => updateExperience(exp.id, 'role', e.target.value)} placeholder="Software Engineer" style={inputStyle} />
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={labelStyle}>Start Date</label>
                        <input type="text" value={exp.startDate} onChange={(e) => updateExperience(exp.id, 'startDate', e.target.value)} placeholder="Jan 2020" style={inputStyle} />
                      </div>
                      <div>
                        <label style={labelStyle}>End Date</label>
                        <input type="text" value={exp.endDate} onChange={(e) => updateExperience(exp.id, 'endDate', e.target.value)} placeholder="Present" style={inputStyle} />
                      </div>
                    </div>
              ...
              <div>
                ...
                {resume.education.map((edu, index) => (
                  <div key={edu.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <input type="text" value={edu.institution} onChange={(e) => updateEducation(edu.id, 'institution', e.target.value)} placeholder="Mumbai University" style={inputStyle} />
                    <label style={labelStyle}>Degree</label>
                    <input type="text" value={edu.degree} onChange={(e) => updateEducation(edu.id, 'degree', e.target.value)} placeholder="Bachelor of Engineering" style={inputStyle} />
                    <label style={labelStyle}>Field</label>
                    <input type="text" value={edu.field} onChange={(e) => updateEducation(edu.id, 'field', e.target.value)} placeholder="Computer Science" style={inputStyle} />
            ...
            {activeSection === 'skills' && (
              <div>
                ...
                <input type="text" placeholder="React, Python..." onKeyDown={(e) => {
              ...
              <div>
                ...
                {resume.projects.map((proj, index) => (
                  <div key={proj.id} style={{ border: '1px solid #e0e0e0', padding: '20px', borderRadius: '10px', marginBottom: '15px' }}>
                    ...
                    <input type="text" value={proj.name} onChange={(e) => updateProject(proj.id, 'name', e.target.value)} placeholder="E-commerce Website" style={inputStyle} />
                    <label style={labelStyle}>Tech</label>
                    <input type="text" value={proj.tech} onChange={(e) => updateProject(proj.id, 'tech', e.target.value)} placeholder="React, Node.js" style={inputStyle} />
                    <label style={labelStyle}>Link</label>
                    <input type="text" value={proj.link} onChange={(e) => updateProject(proj.id, 'link', e.target.value)} placeholder="github.com/project" style={inputStyle} />
                    <label style={labelStyle}>Description</label>
        ...
        <div style={{ background: 'white', borderRadius: '15px', padding: '40px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', minHeight: '800px' }} id="resume-preview">
              ...
              {resume.experience.map(exp => (
                <div key={exp.id} style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <h3 style={{ color: '#1a1a2e', fontSize: '1.1rem' }}>{exp.role || 'Role'}</h3>
                      <p style={{ color: '#667eea', fontWeight: '500' }}>{exp.company || 'Company'}</p>
                    </div>
            ...
            <div style={{ marginBottom: '25px' }}>
              ...
              {resume.education.map(edu => (
                <div key={edu.id} style={{ marginBottom: '15px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <h3 style={{ color: '#1a1a2e', fontSize: '1.1rem' }}>{edu.degree} {edu.field && `in ${edu.field}`}</h3>
          ...
          {resume.skills.length > 0 && (
            <div style={{ marginBottom: '25px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {resume.skills.map(skill => <span key={skill} style={{ background: '#f0f0ff', color: '#667eea', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: '500' }}>{skill}</span>)}
              </div>
            </div>
          )}
